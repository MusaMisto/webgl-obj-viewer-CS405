...
class MeshDrawer {
	...
	constructor() {
		...
		// Task3: Initialize variables for specular lighting
		...
		this.ambientLight = 0.2; // Made it to look more like the result I wanted
		this.specularIntensity = 1.0; // Default specular intensity
	...
	setMesh(vertPos, texCoords, normalCoords) {
		 ...
		 */
		// Task2: Update normal coordinates
	...
	draw(trans) {
		 ...
		 */
		// Task2: Handle lighting
		// Bind and enable the normal buffer
		gl.bindBuffer(gl.ARRAY_BUFFER, this.normalBuffer);
		...
		gl.uniform3f(this.lightPosLoc, lightX, lightY, -20.0); // I set it to make it more akin to the results provided.
		gl.uniform1f(this.ambientLoc, this.ambientLight);
		///////////////////////////////

		// Task3: Set specular lighting uniforms
		gl.uniform1f(this.specularIntensityLoc, this.specularIntensity);
		gl.uniform1f(this.shininessLoc, this.shininess);
		gl.uniform3f(this.viewPosLoc, 0.0, 0.0, 20.0); // Assuming camera is at (0, 0, 20)
	...
	setTexture(img) {
		...
		if (isPowerOf2(img.width) && isPowerOf2(img.height)) {
			 ...
			 */
			// Set texture wrapping for texture
	...
	setAmbientLight(ambient) {
		 ...
		 * @Task2 : You should implement the lighting and implement this function
		 */
		this.ambientLight = ambient / 100;
	}

	setSpecularLight(intensity, shininess) {
		this.specularIntensity = intensity / 100.0; // Normalize to [0, 1]
		this.shininess = shininess; // Shininess directly
	}
...
const meshFS = `
			...
			void main()
			{
				...
				if(showTex && enableLighting){
					...
					// Calculate lighting components and normalize
            		...
            		vec3 diffuseComponent = diffuse * texColor.rgb;
					
					// Calculate reflection vector for specular component
					vec3 reflectDir = reflect(-lightDir, normal);
					float spec = pow(max(dot(viewDir, reflectDir), 0.0), shininess);
					vec3 specularComponent = specularIntensity * spec * texColor.rgb;

					// Combine all lighting components
					vec3 resultColor = ambientComponent + diffuseComponent + specularComponent;
